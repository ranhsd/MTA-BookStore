				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("authorId") === oObject.authorId;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].authorId;
										} else {
											sCurrentId = aItemsNotInTheList[0].authorId;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("Author", {
											authorId : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("authorId"),
							title: oBindingContext.getProperty("authorName")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "Author (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'Author (0)'");
							},
					...
					theListShouldBeSortedAscendingOnUnitNumber : function () {
						return this.theListShouldBeSortedAscendingOnField("numberOfBooks");
					},

					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("authorName");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all Author to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "Author (" + iExpectedLength + ")"}),
									success : function () {
