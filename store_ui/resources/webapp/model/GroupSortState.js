sap.ui.define([
	...
	return BaseObject.extend("com.sap.demo.model.GroupSortState", {
		 ...
		 * @alias com.sap.demo.model.GroupSortState
		 */
		...
		/**
		 * Sorts by authorName, or by numberOfBooks
		 *
		...
		/**
		 * Groups by numberOfBooks, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "numberOfBooks") {
				// Grouping means sorting so we set the select to the same Entity used for grouping
				this._oViewModel.setProperty("/sortBy", "numberOfBooks");

				aSorters.push(
					new Sorter("numberOfBooks", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "authorName");
			}
